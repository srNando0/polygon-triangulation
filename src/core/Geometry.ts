...
export class Circle {
	...
	*/
    public isPointInside(point: Point): boolean {
        return (
            Point.sqrDistance(this.center, point) <=
            this.radius * this.radius + Circle.EPSILON
        );
    }
	...
	*/
    public static circleWith2Points(points: Point[]): Circle {
        if (points.length != 2)
            throw new Error(
                "Number of points different than 2 in Circle.circleWith2Points",
            );
    ...
    public static circleWith3Points(points: Point[]): Circle {
        ...
        const center: Point = Point.add(points[0], c);
    ...
    public static heuristicEnclosingCircle(points: Point[]): Circle {
        ...
        for (const point of points) {
            if (point.pos.x < minXPoint.pos.x) minXPoint = point;

            if (point.pos.x > maxXPoint.pos.x) maxXPoint = point;

            if (point.pos.y < minYPoint.pos.y) minYPoint = point;

            if (point.pos.y > maxYPoint.pos.y) maxYPoint = point;
        }
        ...
        if (distanceX > distanceY)
            circle = new Circle(
                Point.lerp(0.5, minXPoint, maxXPoint),
                distanceX / 2,
            );
        else
            circle = new Circle(
                Point.lerp(0.5, minYPoint, maxYPoint),
                distanceY / 2,
            );
        ...
        for (const point of points) {
            ...
            if (circle.radius < distance) circle.radius = distance;
        }
    ...
    public static welzl(P: Point[], R: Point[]): Circle {
        // Base cases
        if (P.length == 0) {
            if (R.length == 0) return new Circle(new Point(Vector2.zero()), 0);

            if (R.length == 1) return new Circle(R[0], 0);

            if (R.length == 2) return Circle.circleWith2Points(R);
        }

        if (R.length == 3) return Circle.circleWith3Points(R);
        ...
        if (C.isPointInside(p)) return C; // C(P, R) = C(P \ {p}, R)
	...
	*/
    ...
    public isPointInside(p: Point): boolean {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (thisSign * sign != 1) return false;
        }
...
export class Polygon {
    ...
    /*
		Auxiliary Methods
	*/
    /*private static testPotentialEar(
        vertex: DoublyLinkedListNode<Point>,
    ): boolean {
        const triangle: Triangle = Polygon.getTriangleFromEar(vertex);
        return triangle.signedArea() >= 0;
    }*/

    private static getTriangleFromEar(
        node: DoublyLinkedListNode<Point>,
    ): Triangle {
        const previous: DoublyLinkedListNode<Point> | null = node.getPrevious();
        const next: DoublyLinkedListNode<Point> | null = node.getNext();

        return new Triangle(previous.getData(), node.getData(), next.getData());
    }

    /*private static getPotentialEars(
        linkedList: DoublyLinkedList<Point>,
    ): DoublyLinkedListNode<Point>[] {
        const potentialEars: DoublyLinkedListNode<Point>[] = [];

        // Test if the head is a potential ear
        const head: DoublyLinkedListNode<Point> | null = linkedList.getHead();
        if (head == null)
            throw new NullError("head is null in getPotentialEars");

        if (Polygon.testPotentialEar(head)) potentialEars.push(head);

        // Test if other nodes are potential ears
        let node: DoublyLinkedListNode<Point> = head.getNext();

        while (node != head) {
            if (Polygon.testPotentialEar(node)) potentialEars.push(node);

            node = node.getNext();
        }

        return potentialEars;
    }*/

    private static testEar(vertex: DoublyLinkedListNode<Point>): boolean {
        const triangle: Triangle = Polygon.getTriangleFromEar(vertex);

        // If triangle is external, return false
        if (triangle.signedArea() < 0) return false;

        // Get vertex previous to ear
        const previous: DoublyLinkedListNode<Point> = vertex.getPrevious();

        // Get vertex next to the one next to ear
        let node: DoublyLinkedListNode<Point> = vertex.getNext().getNext();

        // Test if vertex is inside the triangle
        while (node != previous) {
            if (triangle.isPointInside(node.getData())) return false;

            node = node.getNext();
        }

        // If no vertex is inside the triangle, then it's a ear
        return true;
    }
	...
	*/
    public earClippingTriangulation(): Triangle[] {
        ...
        const linkedList: DoublyLinkedList<Point> = new DoublyLinkedList<Point>(
            this.vertices,
        );
		...
		*/
        ...
        let iterationsLeft: number = this.vertices.length;
        let node: DoublyLinkedListNode<Point> = head;
        while (linkedList.getSize() >= 3 && iterationsLeft > 0) {
            if (Polygon.testEar(node)) {
                // Case of finding ear
                triangles.push(Polygon.getTriangleFromEar(node));

                const previous: DoublyLinkedListNode<Point> =
                    node.getPrevious();
        ...
        if (iterationsLeft <= 0)
            throw new Error(`More than ${this.vertices.length} iterations`);
