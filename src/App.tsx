...
function drawTest(
    ...
    const center: Point = new Point(
        new Vector2(resolution.width / 2, resolution.height / 2),
    );
    const minResolution: number =
        resolution.height < resolution.width
            ? resolution.height
            : resolution.width;
    const polygon: Polygon = Polygon.randomPolygon(
        center,
        (0.8 * minResolution) / 2,
        numberOfPoints,
        displacement,
    ...
    const list: DoublyLinkedList<Point> = new DoublyLinkedList<Point>(
        polygon.vertices,
    );
...
const App = (): ReactNode => {
	...
	*/
    const clickGenerate = (): void => {
        if (renderer.current == null) throw new NullError("renderer is null");
    ...
    const clickTest = (): void => {
        ...
        for (let i = 1; i <= numberOfTests; i++) {
            ...
            console.log(
                `points: ${numberOfPoints}, heuristic: ${heristic}, smallest: ${smallest}`,
            );
        }
    ...
    return (
        <>
            <div className="p-2 rounded-xl flex flex-col text-center text-xl">
                <canvas
                    ref={canvasRef}
                    className="w-3/4 self-center bg-black rounded-xl"
